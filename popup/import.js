...
class ImportManager {
    ...
    validateFileData(data) {
        ...
        for (const session of data.sessions) {
            if (!session.id || !session.name || !session.domain) {
                return false;
            }
        }
    ...
    // Ganti fungsi handleImport dengan:
    async handleImport() {
        ...
        try {
            // Coba beberapa approach berbeda
            let response;

            // Approach 1: Direct message
            response = await this.sendMessageToBackground({
                action: 'IMPORT_SESSIONS',
                data: JSON.stringify(this.fileData)
            });

            // Approach 2: Coba dengan action yang berbeda
            if (!response || !response.success) {
                response = await this.sendMessageToBackground({
                    action: 'importSessions',  // lowercase
                    data: JSON.stringify(this.fileData)
                });
            }

            // Approach 3: Coba dengan chrome.storage langsung
            if (!response || !response.success) {
                console.log('Trying direct storage access...');
                await this.importDirectly();
    ...
    async importDirectly() {
        return new Promise((resolve, reject) => {
            chrome.storage.local.get(['sessions'], (result) => {
                try {
                    const currentSessions = result.sessions || [];
                    const importedSessions = this.fileData.sessions;

                    // Generate new IDs untuk semua session yang diimport
