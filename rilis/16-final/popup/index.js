...
(() => {
  ...
  var ModalManager = class {
    ...
    setupTabSystem() {
      ...
      const importNewTabPane = document.getElementById("importNewTabPane");
      
      const activateTab = (tabBtn, tabContent) => {
        // Deactivate all tabs
        [exportTabBtn, importTabBtn, importNewTabBtn].forEach(btn => btn?.classList.remove("active"));
        [exportTab, importTab, importNewTabPane].forEach(tab => tab?.classList.remove("active"));
      ...
      if (exportTabBtn) {
        exportTabBtn.addEventListener("click", () => {
          activateTab(exportTabBtn, exportTab);
        });
      }
      
      if (importTabBtn) {
        importTabBtn.addEventListener("click", () => {
          activateTab(importTabBtn, importTab);
        });
      }
      
      if (importNewTabBtn) {
        importNewTabBtn.addEventListener("click", () => {
          activateTab(importNewTabBtn, importNewTabPane);
        });
      }
    }
    getImportFile() {
  ...
  var PopupController = class {
    ...
    setupEventListeners() {
      ...
      getElementByIdSafe("openImportTabBtn").addEventListener("click", () => this.handleOpenImportTab());
    }
    ...
    async handleOpenImportTab() {
      try {
        // Membuat tab baru dengan halaman import khusus
        await chrome.tabs.create({
          url: chrome.runtime.getURL("popup/import.html"),
          active: true
        });
        this.modalManager.hideExportImportModal();
      } catch (error) {
        this.showError(handleError(error, "open import tab"));
      }
    }
  };
